...
async function load(): Promise<void> {
  try {
    ...
    mobileLogger.warn("expense_storage_load_failed", { errorType: classifyError(error) });
  } finally {
    hydrated = true;
    emit();
  }
...
async function persist(): Promise<void> {
  try {
    ...
    mobileLogger.warn("expense_storage_save_failed", { errorType: classifyError(error) });
  }
}
