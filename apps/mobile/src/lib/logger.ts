...
export function classifyError(error: unknown): ErrorKind {
  ...
  if (
    haystack.includes("network") || haystack.includes("fetch") ||
    haystack.includes("offline") || haystack.includes("timeout") ||
    haystack.includes("internet") || haystack.includes("econn") ||
    haystack.includes("err_network") || haystack.includes("failed to fetch")
  ) return "network";

  if (
    status === 401 || status === 403 ||
    haystack.includes("unauthorized") || haystack.includes("forbidden") ||
    haystack.includes("token")
  ) return "auth";

  if (
    status === 400 || status === 422 ||
    haystack.includes("validation") || haystack.includes("invalid")
  ) return "validation";
