...
import { createActionId, logger } from "../lib/logger";
...
const RETRY_QUEUE_STORAGE_KEY = "khata_web_retry_queue_v1";
...
export function useRetryQueue({
  ...
  useEffect(() => {
    ...
    logger.debug("retry_queue_persisted", { queueSize: retryQueue.length });
  ...
  const retryQueuedMutations = useCallback(async () => {
    ...
    for (const queued of snapshot) {
      try {
        ...
        logger.warn("retry_item_failed", { actionId, label: queued.label, attempts: nextAttempts });
      }
    }
