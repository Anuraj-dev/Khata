  ...
  const addExpense = useCallback(
    async (draft: ExpenseDraft): Promise<boolean> => {
      ...
      try {
        ...
        logger.error("add_expense_failed", { actionId, errorType: classifyError(error) });
        return false;
      }
    },
  ...
  const deleteExpense = useCallback(
    async (expenseId: Parameters<typeof deleteExpenseMutation>[0]["expenseId"]): Promise<void> => {
      ...
      try {
        ...
        logger.info("delete_expense_succeeded", { actionId });
      } catch (error) {
        showToast({ kind: "error", message: "Could not delete expense." });
        logger.error("delete_expense_failed", { actionId, errorType: classifyError(error) });
      }
    },
